...
export type RoguePropertyTable =
	typeof(setmetatable(
		{} :: {
			...
			_arrayHelper: any,
			_containerCache: Instance?,
			_observeContainerCache: any,
			_observeDictionaryCache: any,
		},
		{} :: typeof({ __index = RoguePropertyTable })
	))
	& RogueProperty.RogueProperty
...
(RoguePropertyTable :: any).__newindex = function(self, index, value)
	if index == "Value" then
		self:SetValue(value)
...
(RoguePropertyTable :: any).__index = function(self, index)
