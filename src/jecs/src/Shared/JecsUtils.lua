...
function JecsUtils.childOf(r, et)
	return r.jecs.pair(r.jecs.ChildOf, et)
end

function JecsUtils.countQuery(query: Jecst.Query<any>): number
	local count = 0
	for _ in query do
		count += 1
	end
...
function JecsUtils.firstOfQuery(query: Jecst.Query<any>): Jecst.Entity?
...
function JecsUtils.getMaid(r: ImmediateTypes.ImmediateRuntime, entity: Jecst.Entity): Maid.Maid?
	return r.world:get(entity, r.comps.Maid)
end

function JecsUtils.getAMaid(r: ImmediateTypes.ImmediateRuntime, entity: Jecst.Entity): Maid.Maid?
...
function JecsUtils.drainQueuedSnapshot(snapshot: { any }, index: number?): (number?, any?)
...
function JecsUtils.collectObservable(observable: Observable.Observable<any>, maid: Maid.Maid?): RxECSObservableCollector
	local queueOfEmits = {}
	local lastEmit = nil
	...
	return {
		Drain = function()
			...
			queueOfEmits = {}
			return JecsUtils.drainQueuedSnapshot, snapshot, 0
		end,
		Last = function()
...
function JecsUtils.observeComponentOnChanged(
	r: ImmediateTypes.ImmediateRuntime,
	component: Jecst.Id<any>
): Observable.Observable<Jecst.Entity, { curr: any?, removed: boolean }>
	assert(component, `Component not found`)
	return Observable.new(function(sub)
		local maid = Maid.new()
		maid:GiveTask(JecsUtils.addAddCallback(r, component, function(_entity, _id, data)
			sub:Fire(_entity, { curr = data, removed = false })
		end))
		maid:GiveTask(JecsUtils.addChangeCallback(r, component, function(_entity, _id, data)
			sub:Fire(_entity, { curr = data, removed = false })
		end))
		maid:GiveTask(JecsUtils.addRemoveCallback(r, component, function(_entity, _id, _data)
...
function JecsUtils.observeComponentData(
	r: ImmediateTypes.ImmediateRuntime,
	entity: Jecst.Entity,
	component: Jecst.Id<any>
...
function JecsUtils.addChangeCallback(
	r: ImmediateTypes.ImmediateRuntime,
	component: Jecst.Id<any>,
...
function JecsUtils.addAddCallback(
	r: ImmediateTypes.ImmediateRuntime,
	component: Jecst.Id<any>,
	callback: (entity: Jecst.Entity, id: number, data: any) -> (),
	key: any?
...
function JecsUtils.addRemoveCallback(
	r: ImmediateTypes.ImmediateRuntime,
	component: Jecst.Id<any>,
