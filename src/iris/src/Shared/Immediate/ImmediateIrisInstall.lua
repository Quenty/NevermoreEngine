...
export type ImmediateRuntime_Iris<Rt={}> = Rt & ImmediateCoreUtils.ImmediateRuntime & IrisAddon
...
local function resolveIrisParent(r: ImmediateRuntime_Iris): (BasePlayerGui | GuiBase2d)?
...
return function<Rt>(rt: Rt & ImmediateCoreUtils.ImmediateRuntime, scheduler: ImmediateScheduler.ImmediateScheduler?): ImmediateRuntime_Iris<Rt>
	...
	if scheduler and RunService:IsClient() then
		scheduler:RegisterSystem({
			...
			notProtected = false,
			system = function()
				local iris = runtime._raw_iris
				local customParent = runtime.irisParent
				local usingDefaultHost = customParent == nil or customParent.Parent == nil
				local parent = resolveIrisParent(runtime)
				if parent == nil then
					runtime.iris = nil
					return
				end
				...
				runtime.iris = if isIrisDrawable(iris) then iris else nil
			end,
		...
		scheduler:RegisterSystem({
			...
			notProtected = false,
			system = function()
				local _raw_iris = runtime._raw_iris
