...
local MakeBoxSelection = Class 'BoxSelection' (function(BoxSelection, Box2DInterface)
	...
	local Selected = {}
	BoxSelection.SelectionAdded = CreateSignal() -- When a single new item is selected. Will fire many times relative to
	                                           -- SelectionGroupAdded's firing rate.
	BoxSelection.SelectionGroupAdded = CreateSignal() -- When a group is added. Cut down on calcuations..
	BoxSelection.SelectionRemoved = CreateSignal() -- Like above, but when an item is unselected. 
	BoxSelection.SelectionGroupRemoved = CreateSignal() -- Like above, but when an item is unselected. 
	...
	local function Select(self, ...)
		...
		for _, Item in pairs(ToSelect) do
			...
			if not Item.Interfaces.BoxSelection.Selected then
				...
				Item.Box2DRender.Render.ShowSelection()
			end
		end
	...
	local function Unselect(self, ...)
		...
		for _, Item in pairs(ToUnselect) do
			if Item.Interfaces.BoxSelection.Selected then
				...
				Item.Box2DRender.Render.HideSelection()
			end
		end
...
local function MakeTitleFrame(Title, Subtitle, ZIndex)
	...
	local Subtite = Make 'TextLabel' {
		...
		Name                   = "Title";
		Parent                 = TitleFrame;
		Size                   = UDim2.new(1, -TitleSpace, 1, -10);
		Position               = UDim2.new(0, TitleSpace, 1, 0);
		Text                   = Subtitle;
		FontSize               = "Size14";
		TextYAlignment         = "Bottom";
		Visible                = true;
...
local MakeBox2DRenderItem = Class 'Box2DRenderItem' (function(Box2DRenderItem, Item, Box2DInterface)
	...
	local Configuration = {
		...
		SelectedTransparency        = 0.7; -- Transparency when the item is selected. 
	...
	Configuration.CurrentTransparency = DefaultTransparency
	...
	local function AnimateHide(TimePlay)
		...
		qGUI.TweenTransparency(Container, {BackgroundTransparency = 1}, TimePlay, true)
		Update()
	end
...
local MakeBox2DRender = Class 'Box2DRender' (function(Box2DRender, Mouse, Box2DInterface, ScreenGui, Width, Title, Subtitle)
	...
	local ContentFrame = Make 'Frame' { -- Stuff goes in here. 
		...
		Position               = UDim2.new(0, 0, 0, 0);
		Size                   = UDim2.new(1, 0, 1, -Configuration.ScrollbarWidth);
		Visible                = true;
		ZIndex                 = Configuration.ZIndex;
		Archivable             = false;
	}
	...
	local ScrollBarFrame = Make 'Frame' { -- Stuff goes in here. 
		...
		Size                   = UDim2.new(1, 0, 0, Configuration.ScrollbarWidth);
		Visible                = true;
		ZIndex                 = Configuration.ZIndex;
		Archivable             = false;
	}

	local HeaderFrame = MakeTitleFrame(Title, Subtitle)
	...
	local function AddBox2DRenderInterface(Item)
		...
		if not Item.Interfaces.Box2DRender then
			local NewInterface = {}
			NewInterface.Render = MakeBox2DRenderItem(Item, Box2DInterface);
			NewInterface.CurrentYRenderHeight = 0; -- height at which it is rendering... 

			NewInterface.Render.EventBin:add(NewInterface.Render.Gui.MouseButtonClick:connect(function()
				-- Select! Yay!
	...
	local function Update()
		...
		for _, Item in ipairs(VisibleItems) do
			...
			if NewInterface.CurrentYRenderHeight ~= RenderHeightY then
				...
				Item.Interfaces.Box2DRender.Gui:TweenPosition(UDim2.new(0, 0, 0, RenderHeightY), "InOut", "Sine", 0.1, true)
			end
			RenderHeightY = RenderHeightY + Interface.Render.YRenderHeight -- Add to height.
		end
	end
...
local MakeBox2DInterface = Class 'Box2DInterface' (function(Box2DInterface, Mouse, BoxInventory, ScreenGui, Width, Title, Subtitle)
	...
	local Render = MakeBox2DRender(Mouse, Box2DInterface, ScreenGui, Width, Title, Subtitle)
	Box2DInterface.Render = Render
	...
	local function Sort(self, Modes)
		...
		Configuration.SortModes = Modes or Configuration.SortModes;
		ShellSort(VisibleItems, GetValue())
	end
	...
	BoxInventory.ItemAdded:connect(function(Item)
		print("[Box2DInterface] - Adding Item @"..Item.ClassName)
		AddItem(NewItem)
	end)

	BoxInventory.ItemRemoved:connect(function(Item)
		print("[Box2DInterface] - Removing Item @"..Item.ClassName)
		RemoveItem(Item)
	end)
end)
