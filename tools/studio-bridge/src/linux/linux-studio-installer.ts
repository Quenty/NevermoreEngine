...
} from './linux-version-resolver.js';
...
export async function installStudioAsync(
  ...
  for (const [pkg, subdir] of Object.entries(STUDIO_PACKAGES)) {
    ...
    try {
      execSync(`unzip -qo ${JSON.stringify(zipPath)} -d ${JSON.stringify(target)}`, {
  ...
  await fs.writeFile(path.join(studioDir, 'AppSettings.xml'), appSettings, 'utf-8');
