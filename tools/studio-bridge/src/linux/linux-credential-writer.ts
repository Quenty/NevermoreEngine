...
export async function compileWriteCredAsync(
  ...
  try {
    ...
    throw new Error(
      `Failed to compile write-cred.exe: ${
...
export async function injectCredentialsAsync(
  ...
  const prefixExists = await fs
    .access(path.join(config.winePrefix, 'system.reg'))
    .then(
      () => true,
      () => false
    );
  ...
  const entries: Array<[string, string]> = [
    ...
    [`https://www.roblox.com:RobloxStudioAuth${COOKIE_NAME}${userId}`, cookie],
  ];
  ...
  try {
    ...
    OutputHelper.warn(
      `OAuth2 refresh token injection failed (non-fatal): ${
        error instanceof Error ? error.message : error
      }`
    );
  }
...
async function writeRegistryAuthAsync(
  ...
  for (const [keyPath, name, value] of regEntries) {
    ...
    if (result.exitCode !== 0) {
      OutputHelper.warn(
        `Failed to write registry entry ${name} (non-fatal): ${
  ...
  if (userIdResult.exitCode !== 0) {
    OutputHelper.warn(
      `Failed to write UserId registry entry (non-fatal): ${
        userIdResult.stderr || userIdResult.stdout
      }`
    );
  }
}
...
async function injectOAuth2RefreshTokenAsync(
  ...
  if (!tokenResponse.ok) {
    ...
    throw new Error(
      `OAuth token exchange failed: ${tokenResponse.status} ${body}`
    );
  }
  ...
  if (result.exitCode !== 0) {
    const stderr = result.stderr || result.stdout || 'unknown error';
    throw new Error(
      `Failed to write OAuth2 refresh token credential: ${stderr}`
    );
  }

  OutputHelper.verbose(
    'OAuth2 refresh token injected into Wine Credential Manager.'
  );
}
...
async function getCsrfTokenAsync(cookie: string): Promise<string> {
  const response = await fetch(`${ROBLOX_AUTH_API}/v1/authentication-ticket/`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      Cookie: `${COOKIE_NAME}=${cookie}`,
...
async function resolveUserIdAsync(cookie: string): Promise<number> {
  const response = await fetch(`${ROBLOX_USERS_API}/v1/users/authenticated`, {
    headers: {
      Cookie: `${COOKIE_NAME}=${cookie}`,
    },
  });
