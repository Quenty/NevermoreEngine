...
export async function setupHandlerAsync(args: SetupArgs): Promise<SetupResult> {
  try {
    ...
    if (missing.length > 0) {
      ...
      return {
        success: false,
        summary: `Missing prerequisites: ${missing
          .map((m) => m.name)
          .join(', ')}`,
      };
    }
    ...
    const version = await linux.resolveStudioVersionAsync(
      args['studio-version']
    );
    ...
    return {
      success: true,
      summary: `Studio ${version} installed and configured`,
    };
  } catch (err) {
...
export const linuxSetupCommand = defineCommand<SetupArgs, SetupResult>({
  ...
  name: 'setup',
  description:
    'Install Wine + Roblox Studio for headless Linux operation (within Docker image or Linux with Wine)',
  category: 'infrastructure',
  safety: 'none',
  scope: 'standalone',
  args: {
    'install-deps': arg.flag({
      description: 'Install system dependencies via apt-get (requires sudo)',
    }),
    'studio-version': arg.option({
      description: 'Studio version hash (default: latest from CDN)',
    }),
    'studio-dir': arg.option({
      description: 'Override Studio installation directory',
    }),
