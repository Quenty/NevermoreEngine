...
describe('BridgeClient', () => {
  ...
  describe('connectAsync', () => {
    ...
    it('sends list-sessions request on connect', async () => {
      ...
      const listReqs = host.receivedMessages.filter(
        (m) => m.type === 'list-sessions'
      );
  ...
  describe('listSessions', () => {
    ...
    it('returns sessions from host response', async () => {
      ...
      expect(listed.map((s) => s.sessionId).sort()).toEqual([
        'session-a',
        'session-b',
      ]);
    });
  ...
  describe('action forwarding', () => {
    it('sends HostEnvelope for session actions', async () => {
      ...
      const envelopes = host.receivedMessages.filter(
        (m) => m.type === 'host-envelope'
      );
