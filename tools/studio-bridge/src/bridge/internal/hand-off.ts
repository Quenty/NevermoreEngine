 ...
 */
export function computeTakeoverJitterMs(options: {
  graceful: boolean;
}): number {
  if (options.graceful) {
    return 0;
...
export class HandOffManager {
  ...
  async onHostDisconnectedAsync(): Promise<'promoted' | 'fell-back-to-client'> {
    ...
    this._log(
      'Beginning takeover attempt',
      prevState,
      'taking-over',
      graceful ? 'graceful-disconnect' : 'crash-detected'
    );
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      ...
      if (bindSuccess) {
        this._log(
          'Port bind succeeded — promoted to host',
          'taking-over',
          'promoted',
          'bind-success',
          { attempt }
      ...
      const clientConnected = await this._deps.tryConnectAsClientAsync(
        this._port
      );

      if (clientConnected) {
        this._log(
          'Another host detected — falling back to client',
          'taking-over',
          'fell-back-to-client',
          'new-host-found',
          { attempt }
      ...
      if (attempt < MAX_RETRIES - 1) {
        this._log('Retry attempt', 'taking-over', 'taking-over', 'retry', {
          attempt,
