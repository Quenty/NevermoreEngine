...
describe('SessionTracker', () => {
  ...
  describe('addSession', () => {
    ...
    it('tracks multiple sessions', () => {
      ...
      tracker.addSession(
        's2',
        createSessionInfo({ sessionId: 's2', instanceId: 'inst-2' }),
        handle
      );
  ...
  describe('updateSessionState', () => {
    it('updates state and emits session-updated', () => {
      ...
      tracker.addSession(
        's1',
        createSessionInfo({ sessionId: 's1', state: 'Edit' }),
        handle
      );
      ...
      expect(listener).toHaveBeenCalledTimes(1);
      expect((listener.mock.calls[0][0] as TrackedSession).info.state).toBe(
        'Play'
      );
    });
  ...
  describe('listInstances', () => {
    ...
    it('groups sessions by instanceId', () => {
      ...
      expect(instances[0].contexts.sort()).toEqual([
        'client',
        'edit',
        'server',
      ]);
  ...
  describe('getSessionByContext', () => {
    ...
    it('returns undefined for unknown instanceId', () => {
      expect(
        tracker.getSessionByContext('nonexistent', 'edit')
      ).toBeUndefined();
    });
  });
