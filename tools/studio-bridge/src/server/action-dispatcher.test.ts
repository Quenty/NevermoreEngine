...
describe('ActionDispatcher', () => {
  ...
  describe('timeout', () => {
    ...
    it('uses custom timeout when provided', async () => {
      const { responsePromise } = dispatcher.createRequestAsync(
        'queryState',
        500
      );
    ...
    it('uses 30s fallback for unknown action type', async () => {
      const { responsePromise } =
        dispatcher.createRequestAsync('unknownAction');
  ...
  describe('handleResponse', () => {
    ...
    it('rejects pending request when plugin sends error response', async () => {
      const { requestId, responsePromise } =
        dispatcher.createRequestAsync('queryDataModel');
  ...
  describe('cancelAll', () => {
    it('rejects all pending requests', async () => {
      ...
      await expect(req1.responsePromise).rejects.toThrow(
        'Server shutting down'
      );
      await expect(req2.responsePromise).rejects.toThrow(
        'Server shutting down'
      );
    });
