...
import { encodeMessage, decodePluginMessage } from '../../src/protocol.js';

describe('encodeMessage', () => {
  it('encodes a welcome message', () => {
    const json = encodeMessage({ type: 'welcome', payload: { sessionId: 'abc-123' } });
    const parsed = JSON.parse(json);
    expect(parsed).toEqual({ type: 'welcome', payload: { sessionId: 'abc-123' } });
  });
  ...
  it('encodes an execute message', () => {
    const json = encodeMessage({ type: 'execute', payload: { script: 'print("hi")' } });
    const parsed = JSON.parse(json);
    expect(parsed).toEqual({ type: 'execute', payload: { script: 'print("hi")' } });
  });
});
...
describe('decodePluginMessage', () => {
  describe('hello', () => {
    it('decodes a valid hello message', () => {
      const msg = decodePluginMessage(JSON.stringify({
        type: 'hello',
        payload: { sessionId: 'test-session' },
      }));
      expect(msg).toEqual({ type: 'hello', payload: { sessionId: 'test-session' } });
    });
  ...
  describe('malformed messages', () => {
    ...
    it('returns null for unknown message type', () => {
      expect(decodePluginMessage(JSON.stringify({
        type: 'unknown',
        payload: {},
      }))).toBeNull();
    });
