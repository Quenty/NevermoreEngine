...
if (!pkg) {
  console.error('usage: plan.js <pkg> [json] [--eval-gold]');
  process.exit(2);
}

const listRef = evalGold ? 'main' : 'HEAD';
const files = cp
  .execSync(`git ls-tree -r --name-only ${listRef} -- ${pkg}`)
  .toString()
  .trim()
  .split('\n')
  .filter(
...
if (!files.length) {
  console.error(`no source .lua files under ${pkg}`);
  process.exit(1);
}
...
for (const f of files) {
  ...
  adj[f] = [
    ...new Set(
      reqs
        .filter((r) => nameToFile[r] && nameToFile[r] !== f)
        .map((r) => nameToFile[r])
    ),
  ];
  traits[f] = {
...
function isTarget(f) {
  if (evalGold) {
    try {
      return isStrictHeader(cp.execSync(`git show main:${f}`).toString());
    } catch (e) {
      return false;
    }
  }
...
function modelFor(comp) {
  ...
  const hard =
    tgts.some((f) => traits[f].isClass || traits[f].usesRx) || comp.length > 1;
...
const compIndex = {};
sccs.forEach((c, i) => c.forEach((f) => (compIndex[f] = i)));
...
if (asJson) {
  process.stdout.write(JSON.stringify({ pkg, units }, null, 2) + '\n');
  process.exit(0);
}

const mark = (r, u) =>
  u.targets.includes(r) ? r : `${r}  (${skipReason} — skipped)`;
const convertUnits = units.filter((u) => u.targets.length);
console.log(
  `${pkg}: ${files.length} source files -> ${convertUnits.length} convertible units ` +
    `(${
      units.length - convertUnits.length
    } units skipped: ${skipReason}). Dependency order:\n`
);
for (const u of units) {
  if (!u.targets.length) {
    console.log(
      `${String(u.step).padStart(2)}. [skip       ] ${u.files.join(
        ', '
      )}  (${skipReason})`
    );
    continue;
  ...
  const tag = `${
    u.kind === 'single' ? 'single ' : 'cluster'
  } · ${u.model.padEnd(6)}`;
  if (u.kind === 'single')
    console.log(
      `${String(u.step).padStart(2)}. [${tag}] ${mark(u.files[0], u)}`
    );
  else {
    console.log(
      `${String(u.step).padStart(2)}. [${tag}] CLUSTER — convert ${
        u.targets.length
      }/${u.files.length} (cyclic-types playbook)`
    );
...
console.log(
  `\nConvert ${convertUnits.length} units: ${onOpus} opus (class/cyclic/Rx), ${
    convertUnits.length - onOpus
  } sonnet (mechanical).`
);
console.log(
  `Intra-package only; cross-package deps assumed already strict; ${skipReason} files left as-is.` +
    (evalGold
      ? ''
      : '  [real mode: targets = not-yet-strict files; pass --eval-gold for the harness view]')
);
