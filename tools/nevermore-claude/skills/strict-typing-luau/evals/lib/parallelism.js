...
const cases = JSON.parse(
  fs.readFileSync(path.join(HERE, '..', 'parallelism.json'), 'utf8')
).cases;
...
if (process.argv[2] === 'score') {
  ...
  const got = new Map(
    arr.map((o) => [o.n, String(o.action || '').toLowerCase()])
  );
  let fails = 0;
  ...
  console.log('');
  if (fails === 0)
    console.log(`PARALLELISM TEST: all ${cases.length} cases correct`);
  else {
    console.log(`PARALLELISM TEST: ${fails}/${cases.length} MISCLASSIFIED`);
    process.exit(1);
  }
  process.exit(0);
}
