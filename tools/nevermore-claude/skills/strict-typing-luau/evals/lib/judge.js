...
function runScore({ cases, answers, label, cols, slice, show }) {
  ...
  cases.forEach((testCase, index) => {
    ...
    console.log(
      pad(caseNum, 3) +
        pad(testCase.tag, cols.tag) +
        pad(show(testCase.expect), cols.exp) +
        pad(answer === undefined ? '?' : show(answer), cols.got) +
        (ok ? 'ok      ' : 'MISS    ') +
        testCase.prompt.slice(0, slice)
    );
  });
  console.log('');
  if (fails === 0) {
    console.log(`${label}: all ${cases.length} cases correct`);
    return 0;
  }
