...
if (process.argv[2] === "score") {
  ...
  console.log(pad("#", 3) + pad("TAG", 20) + pad("EXPECT", 8) + pad("GOT", 6) + "VERDICT  PROMPT");
  cases.forEach((c, i) => {
    const n = i + 1;
    const g = got.get(n);
    const ok = g === c.expect;
    if (!ok) fails++;
    console.log(
      pad(n, 3) + pad(c.tag, 20) + pad(c.expect ? "fire" : "skip", 8) +
      pad(g === undefined ? "?" : g ? "fire" : "skip", 6) +
      (ok ? "ok      " : "MISS    ") + c.prompt.slice(0, 52)
    );
  });
  console.log("");
  if (fails === 0) console.log(`TRIGGER TEST: all ${cases.length} cases correct`);
  else { console.log(`TRIGGER TEST: ${fails}/${cases.length} MISCLASSIFIED`); process.exit(1); }
  process.exit(0);
}
