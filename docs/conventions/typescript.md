...
- **One concern per file**: Section headers (e.g., `// ── GitHub API ──`) separating **independent concerns** signal a split — each concern should be its own module. A reader should understand a file's purpose from its name alone. For example, GitHub API helpers belong in `github-api.ts`, not above the reporter class that uses them. Don't use section headers to organize internal implementation of a single concern (types, patterns, helpers for one parser) — let the code flow top-to-bottom instead. A stronger signal than headers is **duplication**: if a utility already exists elsewhere, import it rather than inlining a copy.
